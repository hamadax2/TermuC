...
HeaderAdapter.OnChangedListener, View.OnLayoutChangeListener {
	...
	public final static String PWD = "p", SHOWLIST = "l", FILES = "o", TESTAPP = "t", INITAPP = "i", OPEN_FILES = "open_files", OPEN_SELECTED = "open_selected";
		...
		try {
			...
			if (keyboardShown != b && (transTxV == null || !transTxV.isAttachedToWindow())) {
				...
				menu.findItem(R.id.redo).setShowAsAction(
					...
					^ showas);
				menu.findItem(R.id.run).setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
    ...
    private void restoreOpenFiles(SharedPreferences pref) {
        String raw = pref.getString(OPEN_FILES, "");
        if (raw.length() == 0) return;
        String[] paths = raw.split("\\n");
        String selected = pref.getString(OPEN_SELECTED, "");
        for (String path : paths) {
            if (path == null || path.length() == 0) continue;
            File f = new File(path);
            if (FileHelper.isFile(f)) openFile(f);
        }
        if (selected.length() > 0) {
            for (int i = 0; i < hda.getCount(); i++) {
                if (selected.equals(hda.getItem(i))) {
                    selectItem(i);
                    break;
                }
            }
    ...
    private void persistOpenFiles() {
        ...
        for (int i = 0; i < hda.getCount(); i++) {
            if (i > 0) sb.append('\n');
            sb.append(hda.getItem(i));
        }
        int sel = getActionBar() == null ? -1 : getActionBar().getSelectedNavigationIndex();
        String selected = sel >= 0 && sel < hda.getCount() ? hda.getItem(sel) : "";
        getPreferences(MODE_PRIVATE).edit()
            .putString(PWD, pwd.getPath())
            .putString(OPEN_FILES, sb.toString())
            .putString(OPEN_SELECTED, selected)
            .commit();
    }

    @Override
    protected void onStop() {
        // Auto-save the current document before leaving the editor.
        try {
            if (lastFrag != null && lastFrag.getView() != null && lastFrag.getView().isEdited())
                lastFrag.save();
