...
DialogInterface.OnDismissListener, Runnable {
	...
	public void onAdd(CharSequence text, int cursorPosition, int addCount) {
        ...
        mScroller.abortAnimation();
		if (isAutoCompeted) {
			if (cursorPosition - curr >= 2 && Character.isLetterOrDigit(ch))
			//是否开启代码提示
    ...
    private void realDraw(Canvas canvas) {
        ...
        mLeftOffset = isShowLineNumbers ? (int) mTextPaint.measureText("a" + hDoc.getLineCount()) + Math.max(2, mSpaceWidth/4) : 0;
        int paintX;
